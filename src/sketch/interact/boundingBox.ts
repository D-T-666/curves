...
export const interactBoundingBox = (
    p: p5,
    ...
    let cs = [
        p.createVector(
            bb.p1.x - (r + b._draw_params._thickness / world_transforms.scale),
            bb.p1.y - (r + b._draw_params._thickness / world_transforms.scale),
        ),
        p.createVector(
            bb.p2.x + (r + b._draw_params._thickness / world_transforms.scale),
            bb.p1.y - (r + b._draw_params._thickness / world_transforms.scale),
        ),
        p.createVector(
            bb.p1.x - (r + b._draw_params._thickness / world_transforms.scale),
            bb.p2.y + (r + b._draw_params._thickness / world_transforms.scale),
        ),
        p.createVector(
            bb.p2.x + (r + b._draw_params._thickness / world_transforms.scale),
            bb.p2.y + (r + b._draw_params._thickness / world_transforms.scale),
    ...
    if (interaction_vars.pmouseIsPressed) {
        ...
        if (
            hovering < 0 &&
            pmouse.x > rot_anchor.x - r &&
            pmouse.x < rot_anchor.x + r &&
            pmouse.y > rot_anchor.y - r &&
            pmouse.y < rot_anchor.y + r
        )
            hovering = 9;
    }
    ...
    if (p.mouseIsPressed) {
        if (hovering === 8) {
            ...
            else b._anchors.forEach((a) => (a.y = (a.y - c) * s + c));
